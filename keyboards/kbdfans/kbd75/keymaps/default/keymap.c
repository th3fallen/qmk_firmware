...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    case OPENCOVERAGE:
        if (record->event.pressed) {
            // when keycode QMKBEST is pressed
            SEND_STRING(SS_DOWN(X_LCTL) SS_DOWN(X_LSFT) SS_DOWN(X_ZKHK) SS_UP(X_LCTL) SS_UP(X_LSFT) SS_UP(X_ZKHK) SS_DELAY(1000) "~/Sites/When-I-Work/wheniwork-js/reports/coverage/lcov-report/index.html");
        }
        break;
    }
    return true;
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  [0] = LAYOUT(
    MT(MO(1), KC_ESC),   KC_F1,    KC_F2,    KC_F3,    KC_F4,    KC_F5,    KC_F6,    KC_F7,    KC_F8,    KC_F9,    KC_F10,   KC_F11,   KC_F12,   KC_MPRV,  KC_MPLY,  KC_MNXT,
